module yugo::market_core_v2 {
    ...
    public entry fun admin_inject_to_market(admin: &signer, market_addr: address, amount: u64) acquires Market, MarketRegistry, MarketConfig {
    ...
    public entry fun admin_cancel_injection(admin: &signer, market_addr: address, amount: u64) acquires Market, MarketRegistry, MarketConfig {
        let market = borrow_global_mut<Market>(market_addr);
        let now = timestamp::now_seconds();
    ...
    /// Build a short outcome name like "O0", "O1" from index (supports 0-99)
    fun make_outcome_name(index: u8): std::string::String {
        let bytes = vector::empty<u8>();
        // 'O'
        vector::push_back(&mut bytes, 79);
        let i_u64 = (index as u64);
        if (i_u64 >= 10) {
            let tens = (i_u64 / 10) as u8;
            let ones = (i_u64 % 10) as u8;
            vector::push_back(&mut bytes, 48 + tens);
            vector::push_back(&mut bytes, 48 + ones);
        } else {
            vector::push_back(&mut bytes, 48 + (index as u8));
        };
    ...
    /// NOTE: Transaction args cannot pass complex structs like vector<PriceRange>.
    /// Accept a flat vector<u64> as [min0, max0, min1, max1, ...] and rebuild ranges internally.
    public entry fun create_multi_outcome_market(
        creator: &signer,
        price_feed_id: vector<u8>,
        price_ranges_flat: vector<u64>,
        fee_percentage: u64,
        bidding_start_time: u64,
        bidding_end_time: u64,
        maturity_time: u64
        ...
        assert!(bidding_end_time < maturity_time, 1001);
        let flat_len = vector::length(&price_ranges_flat);
        assert!(flat_len % 2 == 0, 1006); // must be pairs
        let num_outcomes = flat_len / 2;
        assert!(num_outcomes >= 2, 1002); // At least 2 outcomes
        let current_time = timestamp::now_seconds();
