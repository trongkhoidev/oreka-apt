...
- **Contracts**:
  ...
  - `market_core_v2.move`: Main binary options contract (market logic, events, settlement, claim, withdraw fee, registry, view functions).
  - `pyth_price_adapter.move`: Oracle price integration (Pyth).
- **Tests**: `tests/market_core_v2_tests.move` – Unit tests for contract logic.
...
```
oreka/
├── sources/                # Move smart contracts (types, market_core_v2, pyth_price_adapter)
