...
const MarketBetPanel: React.FC<MarketBetPanelProps> = ({
  phase, selectedSide, setSelectedSide, bidAmount, setBidAmount, handleBid, isSubmitting, connected, longPercentage, shortPercentage, userPositions, fee
}) => (
  <Box bg="gray.800" p={4} borderRadius="xl" mb={4} borderWidth={1} borderColor="gray.700">
    ...
    <FormControl mb={2} mt={6} color="white">
      ...
      <Input placeholder="Enter amount in APT" bg="gray.800" color="white" borderColor="gray.600" borderRadius="md" mb={3} ml={2} mr={2} value={bidAmount} onChange={e => setBidAmount(e.target.value)} />
    </FormControl>
    <HStack spacing={2} mt={1} mb={2} ml={2} mr={2} alignItems="center" justifyContent="center">
      <Button colorScheme="#0040C1" bg="#0040C1" color="white" _hover={{ bg: "#0040C1" }} width="100%" py={6} mb={3} ml={2} mr={2} onClick={handleBid} isLoading={isSubmitting} loadingText="Placing bid..." isDisabled={!connected || selectedSide === null || phase !== 1}>Betting to rich</Button>
    </HStack>
