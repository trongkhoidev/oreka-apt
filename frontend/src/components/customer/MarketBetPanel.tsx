...
const MarketBetPanel: React.FC<MarketBetPanelProps> = ({
  ...
  // Compute live long/short ratio from current pool amounts
  const computedTotal = totalAmount && totalAmount > 0 ? totalAmount : (longAmount + shortAmount);
  const longPctDisplay = computedTotal > 0 ? (longAmount / computedTotal) * 100 : 50;
  const shortPctDisplay = 100 - longPctDisplay;

  return (
    <Box bg="gray.800" p={4} borderRadius="xl" mb={4} borderWidth={1} borderColor="gray.700">
      {/* LONG/SHORT Ratio */}
      <HStack align="center" spacing={3} w="100%">
        <Text fontSize="sm" fontWeight="bold" color="whiteAlpha.800" whiteSpace="nowrap" mb={4}>{longPctDisplay.toFixed(0)}%</Text>
        <Flex flex="1" align="center" w="100%" h="18px" borderRadius="full" bg="gray.800" border="5px solid" borderColor="gray.400" position="relative" overflow="hidden" boxShadow="inset 0 1px 3px rgba(0,0,0,0.6)" mb={4} p={0}>
          {longPctDisplay === 100 ? (
            ...
            <>
              <Box position="absolute" width={`${longPctDisplay}%`} bgGradient="linear(to-r, #00ea00, #56ff56, #efef8b)" transition="width 0.6s ease" h="100%" display="flex" alignItems="center" justifyContent="flex-end" pr={3} left="0" top="0" zIndex={1} />
              <Box position="absolute" right="0" top="0" h="100%" width={`${shortPctDisplay}%`} bgGradient="linear(to-r, #FF6B81, #D5006D)" transition="width 0.6s ease" display="flex" alignItems="center" justifyContent="flex-start" pl={3} zIndex={0} />
            </>
          )}
        </Flex>
        <Text fontSize="sm" fontWeight="bold" color="whiteAlpha.800" whiteSpace="nowrap" mb={4}>{shortPctDisplay.toFixed(0)}%</Text>
      </HStack>
